...
- [AITER](https://github.com/ROCm/aiter) is AMD’s centralized repository that support various of high performance AI operators for AI workloads acceleration. Attention backends that are using AITER has `_AITER_` in the backend name.
...
vLLM ROCm backend now supports two unified attention implementations: one is Triton unified attention, which is developed by vllm community, and the other is Aiter unified attention. Both implementations follow the same principle but use different kernels—the former invokes the default vLLM Triton attention kernel, while the latter calls the AITER Triton attention kernel.
...
During decode (single-token generation), the primary bottleneck is memory bandwidth—we need to read the entire compressed KV cache to compute attention for just one new query token. To minimize data movement, TRITON_MLA uses a custom `decode_attention_fwd` Triton kernel that **fuses the latent KV expansion with attention computation**.
...
def triton_mla_decode():
    ...
    output = decode_attention_fwd(
        ...
        kv_expansion_weights,       # Learned projection: 576 → full dim
        block_tables,
        context_lens,
        ...
    )
...
def triton_mla_prefill():
    ...
    for chunk_start in range(0, context_len, CHUNK_SIZE):
        ...
        # Applies learned projection: compressed_kv @ W_UK → key
        #                            compressed_kv @ W_UV → value
        ...
        # Compute cross-attention: new queries × cached context chunk
    ...
    # Apply learned compression: key @ W_DK → compressed_kv
    #                           value @ W_DV → (combined with above)
...
**Understanding the Two-Path Trade-off**

Why different strategies for decode vs prefill?

- **Decode is memory-bound**: We access a long KV cache (thousands of tokens) to compute attention for 1 new token. Minimizing memory traffic through fusion is critical.

- **Prefill is compute-bound**: We're computing attention over many tokens (e.g., 3616 × 3616 in the example above). The large matrix multiplications dominate runtime, so we optimize for compute throughput even if it means some extra memory operations.
...
🔧 **Assembly-Level Decode Kernel**

The `mla_decode_fwd` kernel is hand-optimized at the assembly level for AMD's CDNA architecture, taking advantage of:
- **Matrix Core instructions**: Optimized use of AMD Matrix Core (MFMA) instructions for the fused expansion + attention computation
- **Infinity Cache**: Memory access patterns designed to maximize Infinity Cache hit rates on MI200/MI300
- **Wavefront scheduling**: Instruction scheduling tuned for AMD's wavefront execution model (64 threads per wavefront)
- **LDS optimization**: Efficient use of Local Data Share (LDS) for intermediate results

These low-level optimizations enable the decode kernel to achieve significantly better performance than generic Triton implementations on AMD hardware.

🚀 **Variable-Length Flash Attention**

For prefill, the backend uses `aiter.flash_attn_varlen_func`, which efficiently handles **variable sequence lengths** in a single batch. This is particularly valuable in production deployments where different requests have different context lengths. The variable-length support means:
- No padding waste (unlike fixed-length implementations)
- Single kernel launch for mixed-length batch
- Optimal memory utilization

**Advanced Features**

**1. Fine-Grained Prefix Caching**

One of ROCM_AITER_MLA's standout features is its support for `kernel_block_size=1`. This enables **token-level prefix caching** without performance degradation—a unique capability among MLA backends.

Traditional attention backends see performance penalties with small block sizes due to:
- Increased indexing overhead
- More pointer chasing for block table lookups
- Less regular memory access patterns

ROCM_AITER_MLA's assembly kernels are optimized to handle these challenges efficiently. In practice, this means:

```python
# You can use block_size=1 for maximal cache reuse
# No performance penalty compared to block_size=16 or block_size=32!
...
**Benefits**:
- Every token can be prefix-cached independently
- Maximal cache hit rates in multi-turn conversations
- Efficient handling of shared prefixes across different requests
- No need to tune block size for performance vs cache efficiency trade-off

**2. CUDA Graph Support**
...
- **FULL mode**: Entire forward pass captured in a single graph
...
**3. Multi-Turn and Long-Context Optimizations**

The combination of fine-grained prefix caching and CUDA graphs makes ROCM_AITER_MLA particularly effective for:

**Multi-Turn Conversations**:
```
User: "What is machine learning?"
[Entire conversation cached at token level]
User: "How does it differ from traditional programming?"
[Previous context fully reused, only new query processed]
```

**Long-Context Processing** (100K+ tokens):
- Compressed KV cache (576D vs 8192D) makes 100K+ contexts feasible
- Fine-grained caching enables efficient incremental updates
- Chunked prefill prevents OOM while maintaining throughput
...
if type(result) is tuple and return_softmax_lse:
    output, lse = result
    lse = lse.T.contiguous()  # (q_len, num_heads) → (num_heads, q_len)
    return (output, lse)
```
